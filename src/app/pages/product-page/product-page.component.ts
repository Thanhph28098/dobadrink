...
export class ProductPageComponent {
   ...
   getProduct(page: number): void {
      const apiUrl = `https://angular-service.onrender.com/api/products/?_limit=${this.limit}&_page=${page}`
   ...
   ByPrice(min: number, max: number) {
      const apiUrl = `https://angular-service.onrender.com/api/products-price-range?price_min=${min}&price_max=${max}`
      this.http.get(apiUrl).subscribe((res: any) => {
         this.filteredProducts = res.docs
      })
   }
   ...
   currentProduct() {
      const apiUrl = 'https://angular-service.onrender.com/api/products/?_limit=10&page=1'
      this.http.get(apiUrl).subscribe((res: any) => {
         this.filteredProducts = res.docs
      })
   }
   ...
   selectCate(id: any) {
      const apiUrl = `https://angular-service.onrender.com/api/categories/${id}`
      this.http.get(apiUrl).subscribe((res: any) => {
   ...
   getCategory() {
      const apiUrl = 'https://angular-service.onrender.com/api/categories'
      this.http.get(apiUrl).subscribe((res: any) => {
   ...
   onSortOrderChange() {
      if (this.selectedSortOrder) {
         const apiUrl = `https://angular-service.onrender.com/api/products/?_sort=price&_order=${this.selectedSortOrder}`
         this.http.get(apiUrl).subscribe((res: any) => {
            this.filteredProducts = res.docs
         })
      }
